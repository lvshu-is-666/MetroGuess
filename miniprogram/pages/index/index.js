...
Page({
  ...
  startGame() {
    ...
    const { gameMode, difficulty, selectedCityIndex } = this.data
    const settings = MetroGameLogic.DIFFICULTY_SETTINGS[difficulty]

    let availableStations = []
    if (gameMode === 'city') {
      const selectedCity = MetroCommon.CITIES[selectedCityIndex]
      availableStations = this.allStations.filter(s => s.city_cn === selectedCity)
    } else {
      availableStations = [...this.allStations]
    }

    if (availableStations.length < settings.stationCount) {
      MetroCommon.showToast('该城市站点数据不足', 'error')
      return
    }

    const shuffled = availableStations.sort(() => Math.random() - 0.5)
    const selectedStations = shuffled.slice(0, settings.stationCount)
  ...
  checkAutoSolved() {
    ...
    if (autoSolved.length > 0) {
      ...
      autoSolved.forEach(item => {
        MetroCommon.showToast(`自动解锁: ${item.station.station_cn}`, 'success')
      })
    }
  },
